...
class SerialComm:
    ...
    command_duration = (
        2  # (s). Once a command has been going for longer than this, stop
    )
    ...
    mouse_stepped_on_sensors = (
        0  # records at what time the mouse got on the sensors (in ms)
    )
    ...
    def connect_serial(self):
        ...
        if ser.is_open:
            ...
            self.serial is None
            warnings.warn(
                "Could not connet to serial on port: {}".format(self.com_port)
            )
    ...
    def camera_triggers(self):
        ...
        while True:
            ...
            if self.n_arduino_triggers == 0:
                ...
                now = time.time()
                print(
                    "100 arduino triggers in {}s - expected: {}s".format(
                        round(now - start, 2), frame_s * 100
                    )
                )
                start = now
    ...
    def read_serial(self, expected=5):
        ...
        if len(ser_bytes) <= 5:
            return None  # empty string from arduino
    ...
    def setup_pins(self):
        ...
        self.speaker_commad_pin = self.arduino.get_pin(
            "d:{}:o".format(self.arduino_config["tone_pin"])
        )
        self.door_open_pin = self.arduino.get_pin(
            "d:{}:o".format(self.arduino_config["door_open_pin"])
        )
    ...
    def read_arduino_write_to_file(self, camera_timestamp):
        ...
        append_csv_file(
            self.arduino_inputs_file, states, self.arduino_config["arduino_csv_headers"]
        )
    ...
    def live_sensors_control(self, sensors_states):
        ...
        if self.door_status == "closed" and not self.open_command_on:
            ...
            above_th = [
                ch for ch, v in sensors_states.items() if v >= self.live_sensors_ths[ch]
            ]
            ...
            if len(above_th) == self.n_sensors:
                ...
                if (
                    time.time()
                ) - self.mouse_stepped_on_sensors > self.time_on_sensors / 1000:
